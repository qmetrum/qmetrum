...
def generate_quarterly_report(output_path, data):
    ...
    def on_first(c, doc_obj):
        draw_cover_page(c, doc_obj,
            ...
            cover_metrics=[
                ...
                ("REGIME", humanize_label(rm.get("regime", "Normal"))),
    ...
    # ── PERFORMANCE (realized return, benchmark, attribution) ──
    pb = d.get("performance_block") or {}
    if pb.get("periods"):
        def _rp(v):
            return f"{v * 100:+.1f}%" if v is not None else "n/a"
        story.append(Paragraph("PERFORMANCE", s_section))
        story.append(Paragraph("Realized return and benchmark comparison", s_subheading))
        perf_rows = [["Period", "Portfolio", pb.get("benchmark_label") or "Benchmark", "Relative"]]
        for p in pb["periods"]:
            perf_rows.append([p["label"], _rp(p["portfolio"]), _rp(p["benchmark"]), _rp(p["relative"])])
        story.append(styled_table(perf_rows, [0.28, 0.24, 0.28, 0.20]))
        story.append(Spacer(1, 4))
        story.append(Paragraph(escape(pb.get("return_basis", "")), s_disclaimer))

        attr = pb.get("attribution") or []
        if attr:
            story.append(Spacer(1, 8))
            story.append(Paragraph(f"Contribution by holding ({pb.get('attribution_window', '')})", s_subheading))
            a_rows = [["Holding", "Weight", "Return", "Contribution"]]
            for c in attr:
                a_rows.append([
                    f"{c['ticker']}",
                    f"{c['weight'] * 100:.0f}%",
            ...
            # Reconciling residual so the parts sum to the headline return.
            if pb.get("attribution_residual") is not None:
                a_rows.append(["Other / unexplained", "", "", _rp(pb["attribution_residual"])])
            if pb.get("attribution_headline") is not None:
                a_rows.append(["Total portfolio", "", "", _rp(pb["attribution_headline"])])
            story.append(styled_table(a_rows, [0.40, 0.18, 0.20, 0.22]))
        if narrative.get("performance_commentary"):
            story.append(Spacer(1, 6))
            story.append(Paragraph(escape(narrative["performance_commentary"]), s_body_sm))
        story.append(Spacer(1, 12))
    ...
    story.append(Paragraph(
        ...
        f"<b>{humanize_label(rm.get('regime', 'Normal'))}</b> with a fragility score of "
    ...
    risk_table_data = [
        ...
        ["Value at Risk (95%, 1-day)", fmt_pct(rm.get("var_95_daily")),
         "The loss the portfolio is not expected to exceed on 95% of days"],
    ...
    if scenarios:
        ...
        for row in scenarios:
            ...
            sc_table_data.append([
                humanize_label(row["name"]) + (" (reference)" if row.get("is_base") else ""),
                _cell(ret),
        ...
        if sa.get("summary"):
            ...
            for e in sa.get("explanations", []):
                story.append(Paragraph(
                    f"<b>{escape(humanize_label(e.get('name', '')))}</b>: {escape(e.get('narrative', ''))}",
                    s_body_sm
                ))
            story.append(Paragraph(
